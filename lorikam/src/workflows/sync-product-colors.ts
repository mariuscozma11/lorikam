...
export const syncProductColorsStep = createStep(
  ...
  async (input: SyncProductColorsInput, { container }) => {
    ...
    if (colorNames.length > 0) {
      ...
      // Include manage_inventory to inherit it for new color variants
      const refreshedProduct = await productService.retrieveProduct(input.product_id, {
        select: ["id", "variants.id", "variants.title", "variants.manage_inventory"],
      ...
      for (const baseVariant of baseVariants) {
        ...
        // Inherit manage_inventory from base variant (default to false if not set)
        const inheritManageInventory = (baseVariant as any).manage_inventory ?? false
        ...
        for (const colorName of colorNames) {
          ...
          variantsToCreate.push({
            product_id: input.product_id,
            title: variantTitle,
            manage_inventory: inheritManageInventory,
      ...
      // Check if there are any existing variants with color to inherit manage_inventory from
      if (baseVariants.length === 0) {
        // Try to find an existing variant to inherit manage_inventory from
        const existingWithInventory = existingVariants.find((v: any) =>
          typeof (v as any).manage_inventory === 'boolean'
        )
        const defaultManageInventory = existingWithInventory
          ? (existingWithInventory as any).manage_inventory
          : false
        ...
        for (const colorName of colorNames) {
          ...
          variantsToCreate.push({
            product_id: input.product_id,
            title: colorName,
            manage_inventory: defaultManageInventory,
