...
const CustomerDiscountsPage = () => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      resetForm()
      toast.success("Reducerea a fost salvata!")
    },
    onError: (error) => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["customer-discounts"] })
      toast.success("Reducerea a fost stearsa!")
    },
    onError: (error) => {
      toast.error("Eroare: " + (error as Error).message)
    },
  ...
  const handleDeleteDiscount = (customerId: string) => {
    if (confirm("Esti sigur ca vrei sa stergi aceasta reducere?")) {
    ...
    <>
      <Container className="divide-y p-0">
        <div className="flex items-center justify-between px-6 py-4">
          <div>
            <Heading level="h1">Reduceri Colaboratori</Heading>
            <Text size="small" className="text-ui-fg-muted mt-1">
              Atribuie reduceri permanente pentru colaboratori
            </Text>
          </div>
        </div>
        ...
        <div className="px-6 py-4">
          {customers.length === 0 ? (
            <Text className="text-ui-fg-muted">Nu exista clienti inregistrati.</Text>
          ) : (
            <Table>
              <Table.Header>
                <Table.Row>
                    ...
                    <Table.Row key={customer.id}>
                      ...
                      <Table.Cell className="text-right">
                        <div className="flex gap-2 justify-end">
                          ...
                          >
                            {discount ? "Editeaza" : "Adauga reducere"}
                          </Button>
                          {discount && (
                            <Button
                              variant="danger"
      ...
      <Drawer open={isDrawerOpen} onOpenChange={setIsDrawerOpen}>
        <Drawer.Content>
          <Drawer.Header>
            <Drawer.Title>
              {selectedCustomer
                ? `Reducere pentru ${selectedCustomer.first_name || selectedCustomer.email}`
                : "Reducere Client"}
            </Drawer.Title>
          </Drawer.Header>
          <Drawer.Body className="p-4 space-y-6">
            ...
            <div>
              ...
              <Text size="small" className="text-ui-fg-muted mt-1 mb-2">
                Note interne despre aceasta reducere
              </Text>
              <Textarea
                id="notes"
                value={notes}
